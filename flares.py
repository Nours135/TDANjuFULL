...
def flare_detect(G, centrality):
    ...
    for node, cur_cen in sorted(centrality.items(), key=operator.itemgetter(1)):
        ...
        if len(death_candidates) == 0:
            ...
            for idx in death_candidates:
                if idx != elder:
                    # terminate other flares that the node's neighbor belongs to
                    # change the death value to the elder flare's birth value
                    flares[idx].terminate(node, flares[elder].birth)
                    # merge their nodes to the elder flare
                    flares[elder].nodes.update(flares[idx].nodes)
            # add the current node to the elder flare
            flares[elder].nodes.add(node)

        # add a new condition to handle the case when there is only one flare that the node's neighbor belongs to
        if len(death_candidates) == 1:
            # terminate the only flare that the node's neighbor belongs to
            idx = death_candidates.pop()
            # add a new condition to handle the case when the node's centrality is equal to the flare's birth value
            if cur_cen == flares[idx].birth or cur_cen > flares[idx].birth:
                flares[idx].terminate(node, cur_cen)
